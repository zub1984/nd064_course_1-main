...
def post(post_id):
    ...
    if post is None:
        app.logger.error(
            'Article not found, post_id=%s , return 404 page.', post_id)
...
@app.route('/healthz')
def healthcheck():
    app.logger.debug('Received healthz request, %s', healthy)

    if healthy :
        app.logger.info('OK - healthy')
        response = app.response_class(response=json.dumps(
            {"result": "OK - healthy"}), status=200, mimetype='application/json')
    else:
        app.logger.info('ERROR - unhealthy')
        response = app.response_class(response=json.dumps(
            {"result": "ERROR - unhealthy"}), status=500, mimetype='application/json')
...
def metrics():
    ...
    response = app.response_class(
        response=json.dumps({"status": "success", "code": 0, "data": {
                            "db_connection_count": dbCount, "post_count": post_count}}),
        status=200,
